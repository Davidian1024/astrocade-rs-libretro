...
pub extern "C" fn retro_run() {
    ...
    if crate::MEMORY_REQUESTED.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        eprintln!("bytes at $034D0 .. $034F0: {:02x?}", &core.machine.z80.io.mem[0x34D0..=0x34F0]);
        eprintln!("bytes at $02C00 .. $02C15: {:02x?}", &core.machine.z80.io.mem[0x2C00..=0x2C15]);
        eprintln!("byte at $04F2E: {:02x?}", &core.machine.z80.io.mem[0x4F2E]);
        eprintln!("bytes at $03410 .. $03430: {:02x?}", &core.machine.z80.io.mem[0x3410..=0x3430]);
        eprintln!("bytes at $03430 .. $03460: {:02x?}", &core.machine.z80.io.mem[0x3430..=0x3460]);
        eprintln!("byte at $04F32: {:02x?}", &core.machine.z80.io.mem[0x4F32]);
        eprintln!("byte at $04FDE: {:02x?}", &core.machine.z80.io.mem[0x4FDE]);
    }
    ...
    while frame_step_x4 < CYCLES_PER_FRAME_X4 {
        ...
        let prev_4f32 = core.machine.z80.io.mem[0x4F32];
        let prev_4f27 = core.machine.z80.io.mem[0x4F27];
        let prev_4f26 = core.machine.z80.io.mem[0x4F26];
        let prev_4f2e = core.machine.z80.io.mem[0x4F2E];
        let prev_4fdc = core.machine.z80.io.mem[0x4FDC];
        let prev_4f40 = core.machine.z80.io.mem[0x4F40];
        let prev_4fde = core.machine.z80.io.mem[0x4FDE];
        let cycles = core.machine.z80.step();
        core.step_count += 1;
        frame_step_x4 += cycles * 4;

        #[cfg(feature = "debug_logging")]
        if core.machine.z80.io.mem[0x4F27] != prev_4f27 {
            eprintln!(
                "WATCH $4F27: {:02x} -> {:02x} at pc={:#06x} step={} frame={} fstep={}",
                prev_4f27,
                core.machine.z80.io.mem[0x4F27],
                core.machine.z80.pc,
                core.step_count,
                core.frame_count,
                frame_step_x4 / 4,
        ...
        #[cfg(feature = "debug_logging")]
        if core.machine.z80.io.mem[0x4F2E] != prev_4f2e {
            eprintln!(
                "WATCH $4F2E: {:02x} -> {:02x} at pc={:#06x} step={} frame={} fstep={}",
                prev_4f2e,
                core.machine.z80.io.mem[0x4F2E],
                core.machine.z80.pc,
                core.step_count,
                core.frame_count,
                frame_step_x4 / 4,
        ...
        #[cfg(feature = "debug_logging")]
        if core.machine.z80.io.mem[0x4FDC] != prev_4fdc {
            eprintln!(
        ...
        #[cfg(feature = "debug_logging")]
        if core.machine.z80.io.mem[0x4F32] != prev_4f32 {
            eprintln!(
                "WATCH $4F32: {:02x} -> {:02x} at pc={:#06x} step={} frame={} fstep={}",
                prev_4f32,
                core.machine.z80.io.mem[0x4F32],
                core.machine.z80.pc,
                core.step_count,
                core.frame_count,
                frame_step_x4 / 4,
        ...
        #[cfg(feature = "debug_logging")]
        if core.machine.z80.io.mem[0x4FDE] != prev_4fde {
            eprintln!(
                "WATCH $4FDE: {:02x} -> {:02x} at pc={:#06x} step={} frame={} fstep={}",
                prev_4fde,
                core.machine.z80.io.mem[0x4FDE],
                core.machine.z80.pc,
                core.step_count,
                core.frame_count,
                frame_step_x4 / 4,
